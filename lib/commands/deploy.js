...
const backupRemoteFile = async (config, index) => {
  try {
    ...
    //压缩webDir目录下所有文件形成${bakDir}\\${zipFileName} zip文件
...
const unzipRemoteFile = async (config, index) => {
  try {
    ...
    //解压远程压缩包remoteFileName至webDir文件夹 并且删除远程压缩包，原“rm -rf” 改成“del”
    await ssh.execCommand(
      `unzip -o ${remoteFileName} -d ${webDir} && del ${remoteFileName}`
    )
...
// 删除远程压缩包
const removeRemoteZip = async (config, index) => {
  try {
    const { webDir } = config
    const remoteFileName = `${webDir}.zip`

    log(`(${index}) 删除远程压缩包 ${remoteFileName}`)
...
const createTaskList = (config) => {
  ...
  isRemoveRemoteZip && taskList.push(removeRemoteZip)
...
module.exports = {
  ...
  apply: async (env) => {
    if (checkDeployConfigExists()) {
      const config = import(deployConfigPath)
      ...
      if (env) {
        ...
        if (answers.confirm) {
          ...
          succeed(
            `\n\nmo-deploy  SUCCESS !\n\n${underline(projectName)}项目已在${underline(
              envConfig.name
        ...
        error(
          '请使用 mo-deploy -mode 指定部署环境或在配置文件中指定 cluster（集群）地址'
        )
        process.exit(1)
      }
    } else {
      error(
        'mo-deploy.config.js 文件不存，请使用 mo-deploy init 命令创建'
      )
      process.exit(1)
    }
  }
